...
> The function `station_anomalies_plot()` accept four explicit arguments; and output a time series plot, with a line representing one station, such that it plots all time series of temperatures of a country's stations over a time interval on the same plot.
